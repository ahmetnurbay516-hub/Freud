...
// ── Helius RPC endpoint ──
const HELIUS_RPC = "https://mainnet.helius-rpc.com/?api-key=1638de1b-b8f1-4b4b-b086-396705b0355d";
...
function LiveTicker() {
  const [prices, setPrices] = useState(PRICES_DEFAULT);
  useEffect(()=>{
    const fetch_ = async()=>{
      try {
        ...
        setPrices([
          { sym:"BTC", price:`$${d.bitcoin.usd.toLocaleString()}`,      chg:`${d.bitcoin.usd_24h_change>0?"+":""}${d.bitcoin.usd_24h_change.toFixed(2)}%`,      up:d.bitcoin.usd_24h_change>0 },
          { sym:"ETH", price:`$${d.ethereum.usd.toLocaleString()}`,     chg:`${d.ethereum.usd_24h_change>0?"+":""}${d.ethereum.usd_24h_change.toFixed(2)}%`,     up:d.ethereum.usd_24h_change>0 },
          { sym:"SOL", price:`$${d.solana.usd.toLocaleString()}`,       chg:`${d.solana.usd_24h_change>0?"+":""}${d.solana.usd_24h_change.toFixed(2)}%`,       up:d.solana.usd_24h_change>0 },
          { sym:"BNB", price:`$${d.binancecoin.usd.toLocaleString()}`,  chg:`${d.binancecoin.usd_24h_change>0?"+":""}${d.binancecoin.usd_24h_change.toFixed(2)}%`,  up:d.binancecoin.usd_24h_change>0 },
          { sym:"XRP", price:`$${d.ripple.usd.toLocaleString()}`,       chg:`${d.ripple.usd_24h_change>0?"+":""}${d.ripple.usd_24h_change.toFixed(2)}%`,       up:d.ripple.usd_24h_change>0 },
        ]);
      ...
      <div style={{display:"flex",animation:"ticker 22s linear infinite",width:"max-content",height:"100%",alignItems:"center"}}>
        {[...prices,...prices,...prices,...prices].map((p,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:7,padding:"0 20px",height:"100%",whiteSpace:"nowrap"}}>
            <span style={{fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:900,color:"#ffffffee"}}>{p.sym}</span>
            <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:"#ffffffdd"}}>{p.price}</span>
            <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:11,fontWeight:700,color:p.up?"#22c55e":"#ef4444",textShadow:p.up?"0 0 8px #22c55e88":"0 0 8px #ef444488"}}>{p.up?"▲":"▼"} {p.chg}</span>
...
// ── Generate shareable card using Canvas ──
function generateCard(profile, walletAddr) {
  const W=600,H=340;
  const cv=document.createElement("canvas");
  cv.width=W;cv.height=H;
  const ctx=cv.getContext("2d");
  const acol={"LOW":"#22c55e","MEDIUM":"#f97316","HIGH":"#ef4444","CRITICAL":"#dc2626","EXISTENTIAL":"#ff1a1a"}[profile.dangerLevel]||"#00ff88";
  ...
  g.addColorStop(0,acol+"18");g.addColorStop(1,"transparent");
  ctx.fillStyle=g;ctx.fillRect(0,0,W,H);

  // border
  ctx.strokeStyle=acol+"60";ctx.lineWidth=2;
  ctx.strokeRect(1,1,W-2,H-2);

  // top label
  ctx.fillStyle="#ffffff40";ctx.font="bold 11px monospace";
  ctx.fillText("◈ FREUD — SOLANA PSYCHOLOGICAL PROFILER ◈",W/2,28);
  ctx.textAlign="center";

  // archetype icon + name
  const icon={"Exit Liquidity":"🎯","FOMO Chaser":"🚀","Smart Shadow":"🧠","Whale Parasite":"🐋","Revenge Ape":"🦍","Pump.fun Degenerate":"💊","Top Buyer":"📉","Rug Magnet":"🧲","Diamond Hand":"💎","Noise Trader":"📡"}[profile.archetype]||"🎰";
  ctx.font="40px serif";ctx.textAlign="left";ctx.fillText(icon,28,80);
  ctx.font="bold 26px monospace";ctx.fillStyle=acol;
  ctx.fillText(profile.archetype.toUpperCase(),82,68);
  ctx.font="13px monospace";ctx.fillStyle="#00e5ffcc";
  ctx.fillText("DNA: "+profile.walletDNA,82,88);

  // danger badge
  ctx.fillStyle=acol+"30";ctx.beginPath();ctx.roundRect(82,96,profile.dangerLevel.length*9+16,22,4);ctx.fill();
  ctx.strokeStyle=acol;ctx.lineWidth=1;ctx.stroke();
  ctx.fillStyle=acol;ctx.font="bold 11px monospace";ctx.fillText(profile.dangerLevel,90,111);

  // metrics row
  const metrics=[
    {l:"SOL",v:profile.metrics.solBalance.toFixed(3)+"◎",c:"#00ff88"},
    {l:"TXs",v:String(profile.metrics.txCount),c:"#00e5ff"},
    {l:"Tokens",v:String(profile.metrics.tokenCount),c:"#a855f7"},
  ];
  metrics.forEach(({l,v,c},i)=>{
    const x=28+i*120;
    ctx.fillStyle="#ffffff08";ctx.beginPath();ctx.roundRect(x,130,110,46,6);ctx.fill();
    ctx.fillStyle=c;ctx.font="bold 16px monospace";ctx.textAlign="left";ctx.fillText(v,x+8,152);
    ctx.fillStyle="#ffffff60";ctx.font="10px monospace";ctx.fillText(l,x+8,168);
  });

  // score bars
  const bars=[
    {l:"FOMO",v:profile.fomoScore,c:"#f97316"},
    {l:"PANIC",v:profile.panicScore,c:"#ef4444"},
    {l:"RISK",v:profile.riskScore,c:"#00ff88"},
    {l:"SMART",v:profile.smartMoney,c:"#22c55e"},
    {l:"REVENGE",v:profile.revengeScore,c:"#a855f7"},
    {l:"RUG",v:profile.rugExposure,c:"#ff3b3b"},
  ];
  const barW=250,barX=310,barH=8,barGap=22;
  bars.forEach(({l,v,c},i)=>{
    const y=140+i*barGap;
    const vc=v>75?"#ff3b3b":c;
    ctx.fillStyle="#ffffff0f";ctx.beginPath();ctx.roundRect(barX,y,barW,barH,4);ctx.fill();
    ctx.fillStyle=vc;ctx.beginPath();ctx.roundRect(barX,y,barW*(v/100),barH,4);ctx.fill();
    ctx.fillStyle="#ffffff80";ctx.font="10px monospace";ctx.textAlign="left";ctx.fillText(l,barX,y-3);
    ctx.fillStyle=vc;ctx.font="bold 10px monospace";ctx.textAlign="right";ctx.fillText(v,barX+barW,y-3);
  });

  // wallet addr
  const shortAddr=walletAddr.slice(0,8)+"..."+walletAddr.slice(-6);
  ctx.fillStyle="#ffffff25";ctx.font="11px monospace";ctx.textAlign="left";
  ctx.fillText(shortAddr,28,H-36);

  // site url
  ctx.fillStyle="#00ff88aa";ctx.font="bold 13px monospace";ctx.textAlign="right";
  ctx.fillText("freud-phi.vercel.app",W-20,H-36);

  // bottom border line
  ctx.strokeStyle=acol+"40";ctx.lineWidth=1;
  ctx.beginPath();ctx.moveTo(20,H-48);ctx.lineTo(W-20,H-48);ctx.stroke();

  // watermark
  ctx.fillStyle="#ffffff15";ctx.font="bold 11px monospace";ctx.textAlign="left";
  ctx.fillText("🧠 FREUD",28,H-18);
...
function ProfileCard({ profile, wallet, onShare }) {
  ...
  const [therapying, setTherapying] = useState(false);

  const downloadCard = () => {
    const dataUrl = generateCard(profile, wallet);
    const a = document.createElement("a");
    a.href = dataUrl;
    a.download = `freud-${profile.archetype.replace(/\s+/g,"-").toLowerCase()}.png`;
    a.click();
  };
  ...
  const startTherapy = () => {
    ...
    setTimeout(() => {
      // Opening line
      let opening = "";
      if (m.solBalance < 0.01)
        opening = `Good afternoon. I've reviewed your wallet. ${m.txCount} transactions. ${m.solBalance.toFixed(4)} SOL remaining. Please, sit down.`;
      else if (p.riskScore > 80)
        opening = `I've been expecting you. Your on-chain activity has kept me up at night. We have a lot to discuss.`;
      else if (p.archetype === "Diamond Hand")
        opening = `You haven't sold anything in months. That's either discipline or denial. Let's find out which.`;
      else
        opening = `Thank you for coming. I've reviewed your ${m.txCount} transactions and I have... concerns.`;

      // Middle diagnosis
      let diagnosis = "";
      if (p.fomoScore >= 100)
        diagnosis = `You entered ${m.recentTxs} positions last week alone. That's not a strategy — that's a trauma response. When did you stop trusting yourself?`;
      else if (p.panicScore >= 100)
        diagnosis = `I see ${m.burstCount} burst trading events. You panic, you sell, you regret, you rebuy. This is a cycle. A very expensive cycle.`;
      else if (p.rugExposure > 60)
        diagnosis = `${m.tokenCount} tokens. Most of them dead. You keep giving money to anonymous developers at 3am. We need to talk about boundaries.`;
      else
        diagnosis = `Your behavioral patterns suggest someone who knows what they should do, but consistently does the opposite. Classic self-sabotage.`;

      // Therapy plan
      const plans = [];
      if (p.fomoScore > 60) plans.push("✗ Delete Twitter/X for 48 hours before buying");
      if (p.panicScore > 60) plans.push("✗ Stop checking prices every 5 minutes");
      if (m.tokenCount > 10) plans.push("✗ If you can't explain what it does, don't buy it");
      if (m.solBalance < 0.1) plans.push("✓ Keep at least 10% in SOL. Always.");
      if (p.revengeScore > 60) plans.push("✗ Never trade to recover losses. Ever.");
      plans.push("✓ Sleep before making any purchase over $100");
      plans.push("✓ Ask yourself: 'Would I be comfortable if my mother saw this trade?'");

      setTherapy({ opening, diagnosis, plans: plans.slice(0,4) });
      setTherapying(false);
    }, 2000);
  };
  ...
  const shareTherapy = () => {
    if (!therapy) return;
    const t = `🛋️ FREUD just gave me a therapy session based on my Solana wallet:\n\n"${therapy.opening}"\n\nDiagnosis: ${profile.archetype}\nRisk Score: ${profile.riskScore}/100\n\nGet your free diagnosis 👇\nfreud-phi.vercel.app\n#Solana #DeFi #FREUD`;
  ...
  const generateRoast = () => {
    ...
    // Pick roast based on real metrics
    const roasts = [];

    if (m.solBalance < 0.01 && m.txCount > 50)
      roasts.push(`${m.txCount} transactions. ${m.solBalance.toFixed(4)} SOL left. You didn't trade, you donated to the ecosystem. 💸`);

    if (p.fomoScore >= 100 && p.panicScore >= 100)
      roasts.push(`FOMO 100. Panic 100. You buy when everyone's greedy and sell when everyone's fearful. Textbook. Backwards. 📉`);

    if (m.tokenCount > 20 && p.rugExposure > 60)
      roasts.push(`${m.tokenCount} tokens. Most of them rugged. At this point you're not investing, you're funding anonymous devs' vacations. 🏖️`);

    if (p.revengeScore >= 100)
      roasts.push(`Revenge trading detected. You lost money, so you traded more to win it back, and lost more. Classic. The house always wins. 🎰`);

    if (p.smartMoney < 20 && p.riskScore > 75)
      roasts.push(`Smart Money score: ${p.smartMoney}. Risk score: ${p.riskScore}. You have the risk tolerance of a hedge fund and the strategy of a coin flip. 🪙`);

    if (m.txCount >= 100 && m.solBalance < 0.1)
      roasts.push(`${m.txCount} transactions and you're basically broke. The only thing you've consistently produced is gas fees. ⛽`);

    if (p.archetype === "Rug Magnet")
      roasts.push(`Rug Magnet. ${m.tokenCount} tokens held. You have a supernatural ability to find the exact projects that will rug. It's almost impressive. 🧲`);

    if (p.archetype === "FOMO Chaser")
      roasts.push(`You bought the top ${Math.floor(p.fomoScore/15)} times. At this point the market doesn't move you — you move the market. Unfortunately, down. 🚀📉`);

    if (p.diamondHands > 70 && m.solBalance < 0.1)
      roasts.push(`Diamond hands... on worthless bags. Holding forever isn't a strategy, it's denial. Let go. 💎🗑️`);

    if (roasts.length === 0)
      roasts.push(`${m.txCount} transactions analyzed. FREUD is speechless. Your trading pattern defies psychological classification. That's not a compliment. 🤯`);

    // Pick random roast
    const selected = roasts[Math.floor(Math.random() * roasts.length)];
    setTimeout(() => { setRoast(selected); setRoasting(false); }, 1200);
  };

  const shareRoast = () => {
    if (!roast) return;
    const t = `🔥 FREUD just roasted my Solana wallet:\n\n"${roast}"\n\nGet roasted 👇\nfreud-phi.vercel.app\n#Solana #DeFi #FREUD`;
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(t)}`, "_blank");
  };
  ...
  return (
    <div style={{animation:"fu .6s ease"}}>
      ...
      <div style={{marginBottom:14}}>
        <button onClick={generateRoast} disabled={roasting}
          style={{width:"100%",background:"linear-gradient(135deg,#f9731622,#ef444415)",border:"1px solid #f9731666",color:"#f97316",fontFamily:"'Orbitron',monospace",fontSize:13,fontWeight:800,letterSpacing:1.5,padding:"13px",borderRadius:9,cursor:"pointer",textShadow:"0 0 8px #f9731670",marginBottom:8}}>
          {roasting?"ANALYZING YOUR PAIN...":"🔥 ROAST ME"}
        </button>

        {roasting&&(
          <div style={{textAlign:"center",padding:"12px 0"}}>
            <div style={{width:32,height:32,margin:"0 auto 8px",border:"2px solid #f9731620",borderTop:"2px solid #f97316",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
            <div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:"#f97316",letterSpacing:2}}>FREUD IS JUDGING YOU...</div>
          </div>
        )}

        {roast&&!roasting&&(
          <div style={{background:"linear-gradient(135deg,#1a0800,#0d0500)",border:"1px solid #f9731630",borderRadius:10,padding:"16px",animation:"fu .5s ease"}}>
            <div style={{fontFamily:"'Orbitron',monospace",fontSize:9,color:"#f97316",letterSpacing:2,marginBottom:10}}>🔥 FREUD SAYS:</div>
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:14,color:"#ffffffee",lineHeight:1.8,marginBottom:12}}>"{roast}"</div>
            <button onClick={shareRoast}
              style={{width:"100%",background:"#f9731620",border:"1px solid #f9731650",color:"#f97316",fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:800,padding:"10px",borderRadius:7,cursor:"pointer",letterSpacing:1}}>
              SHARE THIS ROAST ON 𝕏
            </button>
          </div>
        )}
      </div>
      ...
      {/* THERAPY SECTION */}
      <div style={{marginBottom:14}}>
        <button onClick={startTherapy} disabled={therapying}
          style={{width:"100%",background:"linear-gradient(135deg,#3b82f622,#a855f715)",border:"1px solid #3b82f666",color:"#3b82f6",fontFamily:"'Orbitron',monospace",fontSize:13,fontWeight:800,letterSpacing:1.5,padding:"13px",borderRadius:9,cursor:"pointer",textShadow:"0 0 8px #3b82f670",marginBottom:8}}>
          {therapying?"FREUD IS TAKING NOTES...":"🛋️ START THERAPY SESSION"}
        </button>

        {therapying&&(
          <div style={{textAlign:"center",padding:"12px 0"}}>
            <div style={{width:32,height:32,margin:"0 auto 8px",border:"2px solid #3b82f620",borderTop:"2px solid #3b82f6",borderRadius:"50%",animation:"spin 1s linear infinite"}}/>
            <div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:"#3b82f6",letterSpacing:2}}>ANALYZING TRAUMA PATTERNS...</div>
          </div>
        )}

        {therapy&&!therapying&&(
          <div style={{background:"linear-gradient(135deg,#030d1a,#06000f)",border:"1px solid #3b82f630",borderRadius:10,padding:"18px",animation:"fu .5s ease"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14}}>
              <span style={{fontSize:20}}>🛋️</span>
              <div style={{fontFamily:"'Orbitron',monospace",fontSize:10,color:"#3b82f6",letterSpacing:2}}>DR. FREUD — THERAPY SESSION</div>
            </div>

            {/* Opening */}
            <div style={{background:"#3b82f610",borderLeft:"2px solid #3b82f6",borderRadius:"0 6px 6px 0",padding:"10px 14px",marginBottom:12}}>
              <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:"#ffffffee",lineHeight:1.8,fontStyle:"italic"}}>
                "{therapy.opening}"
              </div>
            </div>

            {/* Diagnosis */}
            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:13,color:"#ffffffcc",lineHeight:1.8,marginBottom:14}}>
              {therapy.diagnosis}
            </div>

            {/* Therapy plan */}
            <div style={{background:"#ffffff06",borderRadius:8,padding:"12px",marginBottom:14}}>
              <div style={{fontFamily:"'Orbitron',monospace",fontSize:9,color:"#3b82f6aa",letterSpacing:2,marginBottom:10}}>◈ YOUR THERAPY PLAN</div>
              {therapy.plans.map((plan,i)=>(
                <div key={i} style={{fontFamily:"'Share Tech Mono',monospace",fontSize:12,color:plan.startsWith("✓")?"#22c55e":"#ef4444cc",padding:"4px 0",lineHeight:1.6}}>
                  {plan}
                </div>
              ))}
            </div>

            <div style={{fontFamily:"'Share Tech Mono',monospace",fontSize:11,color:"#ffffff40",marginBottom:12,fontStyle:"italic"}}>
              — Dr. FREUD, On-Chain Psychologist
            </div>

            <button onClick={shareTherapy}
              style={{width:"100%",background:"#3b82f620",border:"1px solid #3b82f650",color:"#3b82f6",fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:800,padding:"10px",borderRadius:7,cursor:"pointer",letterSpacing:1}}>
              SHARE MY DIAGNOSIS ON 𝕏
            </button>
          </div>
        )}
      </div>
      ...
      <div style={{display:"flex",gap:8}}>
        <button onClick={onShare} style={{flex:1,background:"linear-gradient(135deg,#a855f720,#00e5ff10)",border:"1px solid #a855f766",color:"#a855f7",fontFamily:"'Orbitron',monospace",fontSize:12,fontWeight:800,letterSpacing:1.5,padding:"13px",borderRadius:9,cursor:"pointer",textShadow:"0 0 8px #a855f770"}}>
          SHARE ON 𝕏
        </button>
        <button onClick={downloadCard} style={{flex:1,background:"linear-gradient(135deg,#00ff8820,#00e5ff10)",border:"1px solid #00ff8866",color:"#00ff88",fontFamily:"'Orbitron',monospace",fontSize:12,fontWeight:800,letterSpacing:1.5,padding:"13px",borderRadius:9,cursor:"pointer",textShadow:"0 0 8px #00ff8870"}}>
          🖼 SAVE CARD
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [leaderboard, setLeaderboard] = useState(() => {
    try { return JSON.parse(localStorage.getItem("freud_lb") || "[]"); } catch(e) { return []; }
  });
  const [lbTab, setLbTab] = useState("shame");

  const addToLeaderboard = (addr, profile) => {
    const entry = {
      addr: addr.slice(0,6)+"..."+addr.slice(-4),
      full: addr,
      arch: profile.archetype,
      danger: profile.dangerLevel,
      emo: profile.dominantEmotion,
      risk: profile.riskScore,
      smart: profile.smartMoney,
      fomo: profile.fomoScore,
      dna: profile.walletDNA,
      sol: profile.metrics.solBalance,
      txs: profile.metrics.txCount,
      t: new Date().toLocaleTimeString(),
    };
    setLeaderboard(prev => {
      const filtered = prev.filter(e => e.full !== addr);
      const updated = [entry, ...filtered].slice(0, 50);
      try { localStorage.setItem("freud_lb", JSON.stringify(updated)); } catch(e) {}
      return updated;
    });
  };
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      setProfile(result);
      addToLeaderboard(wallet.trim(), result);
    } catch(e) {
    ...
    {page==="home"&&(<>
      ...
      <div style={{maxWidth:1060,margin:"0 auto",padding:"0 20px 48px"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,flexWrap:"wrap",gap:8}}>
          <div>
            <div className="mono" style={{fontSize:10,color:"#ef4444aa",letterSpacing:3,marginBottom:3}}>◈ LEADERBOARD ◈</div>
            <div className="orb" style={{fontSize:17,fontWeight:900,color:"#fff"}}>HALL OF SHAME & FAME</div>
          </div>
        ...
        <div style={{display:"flex",gap:6,marginBottom:12}}>
          {[{id:"shame",label:"🔴 HALL OF SHAME",col:"#ef4444"},{id:"fame",label:"🟢 HALL OF FAME",col:"#22c55e"}].map(({id,label,col})=>(
            <button key={id} onClick={()=>setLbTab(id)}
              style={{flex:1,padding:"10px",borderRadius:8,cursor:"pointer",fontFamily:"'Orbitron',monospace",fontSize:10,fontWeight:800,letterSpacing:.8,
                background:lbTab===id?`${col}20`:"#ffffff06",
                border:`1px solid ${lbTab===id?col+"60":"#ffffff15"}`,
                color:lbTab===id?col:"#ffffff60",transition:"all .2s"}}>
              {label}
            </button>
          ))}
        </div>
        ...
        {leaderboard.length === 0 ? (
          <div style={{background:"#ffffff05",border:"1px solid #ffffff0a",borderRadius:12,padding:"32px",textAlign:"center"}}>
            <div style={{fontSize:32,marginBottom:10}}>🧠</div>
            <div className="orb" style={{fontSize:13,color:"#ffffff60",marginBottom:6}}>NO SCANS YET</div>
            <div className="mono" style={{fontSize:12,color:"#ffffff40"}}>Be the first! Scan a wallet to appear on the leaderboard.</div>
          </div>
        ) : (
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            {[...leaderboard]
              .sort((a,b) => lbTab==="shame" ? b.risk-a.risk : b.smart-a.smart)
              .slice(0,10)
              .map((w,i)=>{
                const dc=DC[w.danger]||"#fff";
                const isTop = i === 0;
                return(
                  <div key={i} style={{background:isTop?"linear-gradient(135deg,#ff3b3b08,#00000000)":"#ffffff05",border:`1px solid ${isTop?"#ef444430":"#ffffff0a"}`,borderRadius:10,padding:"11px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",animation:`fu .5s ease ${i*.04}s both`,position:"relative"}}>
                    {isTop&&<div style={{position:"absolute",top:-8,left:12,background:lbTab==="shame"?"#ef4444":"#22c55e",color:"#000",fontFamily:"'Orbitron',monospace",fontSize:8,fontWeight:900,padding:"2px 8px",borderRadius:10,letterSpacing:1}}>{lbTab==="shame"?"MOST DEGENERATE":"SMARTEST"}</div>}
                    <span className="orb" style={{fontSize:11,color:isTop?(lbTab==="shame"?"#ef4444":"#22c55e"):"#ffffff30",width:18,flexShrink:0,fontWeight:900}}>#{i+1}</span>
                    ...
                    <div style={{textAlign:"right",flexShrink:0}}>
                      <div className="orb" style={{fontSize:16,fontWeight:900,color:lbTab==="shame"?"#ef4444":"#22c55e"}}>{lbTab==="shame"?w.risk:w.smart}</div>
                      <div className="mono" style={{fontSize:9,color:"#ffffff40"}}>{lbTab==="shame"?"RISK":"SMART"}</div>
                    </div>
